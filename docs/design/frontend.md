...
| `/collections` | Collections | 合集管理列表（名称/来源徽章 tmdb_collection/wikidata/manual/作品数），支持搜索、新建、重命名（title_cn 必填 + title_en/description）、删除（仅解除作品关联）；点击名称打开详情抽屉：收录作品列表（链接到剧集/电影详情页，可移除）、`untracked_parts` 未收录区块（`include_parts=true` 按需拉取，灰色 Tag 展示、不可点击）、"挂载作品"弹窗（剧集/电影 tab + 防抖搜索，走 works API 翻页选择） |
...
- **资源详情抽屉**：Channel 详情点击资源行打开右侧抽屉，展示 poster（若有）、metadata（作品名、集数）、解析字段明细、磁力链接复制按钮、"修正 metadata"按钮；点击修正进入手动 metadata 流程：输入搜索词+选择类型→查看 LLM 候选→确认→自动刷新该资源及其相关分组。TV 单集资源在集号行右侧提供铅笔按钮，弹出 Popover 手动修正 `episode` / `absolute_episode`，保存后 `episode_confidence="manual"` 并重新触发 Agent 过滤；填写绝对集号且用户未手动填季号时，前端用 metadata 端点返回的剧集 `seasons` 逐季集数在本地推导并预填季号（与后端 `locate_absolute_episode` 同一算法，越界则留空交给服务端标记）。
