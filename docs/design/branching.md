...
- **CI Strict Gate**（`.github/workflows/ci-strict.yml`）：`develop`、`release/**` 分支及其 PR 的严格门禁——lint + 单元/API（覆盖率 ≥ 80%）+ 集成测试（单节点 `docker-compose.test.yml` 的 app 服务在 coverage 下运行，测试后 `stop app` 落盘并由 `coverage-report` 服务校验 `app/` ≥ 70%）。
