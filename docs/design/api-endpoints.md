...
完整语义（模型、聚合状态机、payload 契约、fan-out 与退避策略、下游清理 API）见 [notifications.md](notifications.md)。投递为纯出站：无 token、无消费者回调端点（旧版 start/ack/fail 已删除）。**快照契约版本 `version: 2`**（作品单季化，破坏性变更）：`work.seasons` 删除、`work.episodes[]` 去 season 分量、`work` 增加 `season_number`/`number_of_episodes`；v1 快照无 `version` 键，下游迁移指引见 notifications.md「快照 payload（契约）」。
...
| POST | `/metadata/search` | 统一只读搜索：`{query, content_type, mode, source?, trusted_sites?}`；`trusted_sites=null` 使用默认顺序，`[]` 禁用全网回退；返回本地或在线规范化候选。**`source=bangumi` 时为列表模式**：不走 search-then-judge 收敛管线（不调 LLM），`list_bangumi_subjects` 多查询合并去重（limit 20、仍限动画分类 type=[2]），每个 subject 返回一条薄候选（身份 `bangumi:{id}`、标题/年份/海报/简介，`metadata.subject_season` 标注条目自身季标记——含 Stage 序数——供前端对照 season_hint）；剧场版 platform 条目 content_type=movie，受请求 content_type 边界过滤 |
| POST | `/works/metadata/preview` | 预览在线候选与现有作品的字段差异及人工保护状态，不写库。bangumi 列表模式的薄候选（metadata 缺 `episode_list`）先按需展开——`build_entity_for_subject` 按 subject id 取详情+剧集端点补齐完整 entity（season 取作品自身 `season_number`）再比对；展开失败保留薄候选，diff 相应变少 |
| POST | `/works/metadata/apply` | 应用已确认候选；同步候选提供的非人工字段，空值不清空，身份冲突返回 409。bangumi 薄候选同样按需展开（与 preview 共享同一份展开值，不重复取网），展开后的 `episode_list` 照常 upsert Episode 行 |
| PATCH | `/resources/{id}/episode` | 手动修正集号：`{ "episode": int|null, "season": int?, "absolute_episode": int|null?, "note": string? }` → 写入 per-season episode（可选保留 absolute_episode），设置 `episode_confidence="manual"`。未显式发送 `season` 且已知 absolute 集号、资源已链接剧集时，服务端沿该作品的**合集成员**定位（`locate_absolute_episode_in_collection`：成员按 `season_number` 累加 `number_of_episodes`）并把资源**重指向**定位到的季作品（legacy 未拆分行回退其自身逐季集数）；episode 也未显式发送时一并推导——显式值永远优先。**先 commit 再入队**（worker 只读已提交数据，避免读到修正前的 `ambiguous` 而重建过期决策），然后对该 channel 下所有 active Agent 入队一次**定向运行**（`resource_ids=[该资源]`）：按 Agent 当前规则只处理该资源，**绕过消费水位线**（资源可能较旧）、**不推进水位线**。省略 `absolute_episode` 时保留原值。提交前对同作品+同频道仍 `ambiguous/raw` 的兄弟资源套用新建立的编号约定批量修复（`heal_sibling_episodes`）， healed 行随本次定向运行一并派发（`resource_ids=[该资源, *healed]`）。三个修订端点（本行与下两行）均在 commit 之后、入队之前对 `torrent_file` 缺失的 http(s) 直链资源 best-effort 重建 torrent 缓存（`ensure_torrent_cached` + `maybe_inspect_torrent`，失败静默不阻断主流程），使文件映射随定向重跑一并重建。 |
| PATCH | `/resources/{id}` | 人工修订解析字段（`ResourceParseCorrectionRequest`）：`{ "episode"?, "season"?, "absolute_episode"?, "episode_start"?, "episode_end"?, "is_batch"?, "batch_scope"?: "season"\|"multi_season"\|"franchise"\|"movies", 通用字段?: resolution/subtitle_group/source/video_codec/audio_codec/subtitle_type/container/subtitle_langs }` —— 全可选，仅显式发送的字段被更新（`model_fields_set` 语义）。服务端强制合集不变量（与抓取期 pre-parser 对齐）：`is_batch=true` 强制 `episode=null` 且未显式发送 scope 时缺省 `batch_scope="season"`；`is_batch=false` 清空 `batch_scope`/`episode_start`/`episode_end`。显式发送 `episode`/`season`/`absolute_episode` 任一即置 `episode_confidence="manual"`；未发送集号字段但修订结果为 `is_batch=true` 且原为 `ambiguous` 时同样置 `manual`（合集无单集概念，标记合集即了结集号/季号问题）；**通用媒体描述字段不触碰 `episode_confidence`**。**先 commit 再入队**对该频道全部 active Agent 的定向运行（语义同 PATCH `/episode`，发送集号字段时同样先跑 `heal_sibling_episodes` 批量修复兄弟资源）。旧 `PATCH /resources/{id}/episode` 保留。 |
| PUT | `/resources/{id}/associations` | 编辑向导统一提交（`ResourceAssociationUpdateRequest`）：`{ "is_batch": bool, "works": [{work_type, work_id}], "collection_id"?, "assignments": [{file_path, work_type, work_id, file_size?, season?, episode_start?, episode_end?}], "season"?/"episode"?/"absolute_episode"?(仅非合集分支), "fields"?: 通用字段及 title_cn/title_en/search_title }` → 原子应用全部不变量：非合集至多一个作品（写入互斥 FK，清空 links/assignments/collection）；合集恰一作品时镜像到 FK（保 dedup coverage key），多作品清 FK 仅留 links；`batch_scope` 自动推导（全 movie→movies；tv+movie 或多 tv→franchise；单 tv→season/multi_season 按季证据）；单季合集从 assignments 同步资源级 `season/episode_start/episode_end`，跨季合集清空这些扁平字段并使用 `batch_seasons/season_ranges`（两者为从关联季作品派生的冗余缓存）；assignments 必须引用 works 集合、TV 文件必须带季、同 (work, season) 集号区间**重叠→422**、断档→200+warnings；**两级关联（TV 先合集后季作品）**：提交 `collection_id` 时 `works` 中每个剧集作品必须属于该合集（不一致 422）；**多作品合集（>1 作品）文件清单完整性**：每个关联作品至少一条指派、已知 torrent 清单的媒体文件全部被指派（清单不可得时该子项降级 warning）、各季作品指派区间覆盖其 `number_of_episodes` 应有集数（整段缺失 422、首尾断档 200+warnings）；works 在线候选物化时若季号不可定（upsert 返回 None）→ 422 提示先关联合集下的具体季作品。work links 与 assignments 均按差量更新，重复保存幂等。works 为空的非合集提交保留现有挂载（含音频链接）。先 commit 再定向 Agent 重跑；响应为 Detail schema + `warnings[]` |
...
| Method | Path | 说明 |
|--------|------|------|
| GET | `/works` | 作品列表（分页，支持 search 模糊搜索和 content_type 过滤：all/tv/movie；`collection_id` 参数：合集 UUID=仅该合集成员（音频作品被排除），字面量 `none`=仅未分组作品，缺省=不过滤；tv/movie 条目带 `collection_id`/`collection_name`（显示名 = 合集 title_cn 或 title_en，未分组为 null）与 `is_anime`（可空布尔三态，见下），剧集条目另带 `season_number`（作品单季化：该作品是第几季，0=特典），音频条目无合集恒为 null） |
| POST | `/works/merge` | 手动合并作品（作品单季化的常规修复工具，见 data-models.md 去重与 per-season-works.md）：body `{survivor_type: "series"\|"movie", survivor_id, duplicate_ids, confirm: true}` → 复用去重合并机器把 `duplicate_ids` 合并进 `survivor_id` 并删除重复行（FileResource/AgentWork/ChannelRawTitleMapping/PendingDecision/Episode/合集 links/文件 assignments 全部重指向，身份袋取并集）。仅同类型可合并，series 另限同 `season_number`（作品即季，跨季合并会破坏模型，422）；`confirm` 非 true → 422 带不可逆警告文案（前端据此弹确认框）；survivor_id 出现在 duplicate_ids / 列表为空 → 422；任一 id 不存在 → 404。响应带各项重指向计数与 notes |
