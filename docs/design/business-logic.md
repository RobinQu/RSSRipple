...
fetch_channel_resources(channel, db)
  ...
  │     │     # 并清空 episode；全N季/N-M季整季标记（detect_season_span）
  │     │     # 额外落 batch_scope="multi_season" + batch_seasons=覆盖季集合
  │     │     #（覆盖度已知即可派发，torrent 分析可修正）
...
- `normalize_parsed_fields(title_raw, parsed)` 对 LLM 生成的 field_mapping 常见 regex miss 做**保守修复**：仅当 `title_cn`/`title_en` 泄漏方括号（多 bracket 标题只剥离了首个 `[...]`）时，从 raw title 重切作品名分段回填（`search_title` 优先取 latin 段）；并只在 tech 字段（resolution/source/video_codec/audio_codec/container）为 None 时从 raw title 补齐。已干净解析的资源不受影响。字幕组规范字段为 `subtitle_groups` 列表；联合发布先按连接符拆候选，无法由已注册名称确定时保留原值并标记 unresolved，再由受约束 judge 选择候选，确认结果自动登记映射表。
...
- 同模块还提供抓取期预解析器：`detect_batch`（合集识别；除既有集数区间/批次关键字/整碟季包规则外，含整季标记 `全N季`（全+汉字/阿拉伯数字+季，如"全六季"）与 `N-M季`（如"1-6季"）——`季` 后缀强制，"全12话"/年份对 `[2020-2024]` 不误判；`detect_season_span` 返回覆盖季集合（全N季→1..N），抓取管线据此落 `batch_scope="multi_season"` + `batch_seasons`）、`detect_absolute_episode`（`NN(MM)` 双标记提取）、`detect_subtitle_langs`（字幕语言 BCP-47 标签）、`strip_season_from_title`（去尾部季后缀，含 Stage 序数/序数词后缀——" Second Stage"/" 2nd Stage"/" Final Stage" 同样剥离，两级标题兜底与 bangumi `_autolink_subject` 基础名比对据此对 Stage 命名生效），语义详见 data-models.md 的合集与集号 reconciliation 章节。共享季标记入口 `_season_marker`（`season_from_title` 与 `extract_season_episode_from_path` 共用）另识别英文序数 Stage 季标记（`(first|second|…|tenth) Stage` → 1..10，兼容 `2nd Stage` 数字序数；**"Final Stage" 刻意不映射季号**——词面推不出季号，猜错比不猜糟）。
...
**Bangumi 频道源匹配流程（`metadata_bangumi`，镜像 wikipedia 的 search-then-judge 形态）**：复用 wikipedia 的 query 清洗（`_candidate_queries`）→ `POST /v0/search/subjects` 限 `type=[2]`（动画分类，命中即动漫）→ 确定性 auto-link（归一化标题相等 + 标题年份守卫 ±1，**唯一命中**才免 LLM；**季感知**——Bangumi 一个 subject 就是一季，资源/刷新提示带季号 >1 时基础名条目（恒为第一季）永不 auto-link，仅"候选标题自带同季标记且基础名命中查询"的唯一候选可免 LLM，其余交 judge）→ 否则单次 LLM judge（专用 prompt `_BANGUMI_JUDGE_SYSTEM_PROMPT`）→ 命中条目经详情（`GET /v0/subjects/{id}`）+ 剧集（`GET /v0/episodes` 分页）端点展开为 matched_entity：`external_id=bangumi:{id}`（身份证据自动落 `is_anime=True`）、title_cn=name_cn、original_title=name、description=summary、rating=rating.score、start_date=date、number_of_episodes=eps、genre=tags（出口钳制）、`is_anime=True`；TV 条目不伪造 `seasons`/`number_of_seasons`，而携带仅在本次匹配链路使用的 `single_season_entry=true`——Bangumi 的一个 subject 本身就是一季（作品单季化后该语义固化为注册表粒度 `granularity="season"`：`bangumi:{id}` 身份直接落**季作品**袋，匹配即得对应季作品），因此标题无季标记时可由统一缺季规则安全补季号，同时不污染作品级总季数；platform=剧场版时不写该证据。episode_list 取本篇（type=0）条目：集号优先用季内 `ep`，缺 `ep` 回退全局 `sort`——后续季 subject 常沿用全系列连续编号（如第三季条目 sort=25-36 配 ep=1-12），若整列最小值 >1 则按最小值 rebase 为 1..N（`_episode_list_from`），同时携带 `airdate` → `air_date`；season 标签用资源解析出的季号（无则 1）；platform=剧场版 → content_type=movie。found=False 走统一有序网络搜索回退（仅补身份）；缓存命名空间 `metadata_agent:bangumi`；手动搜索/评测（`process_title_only`）同样支持，手动搜索响应保留 `single_season_entry` 供链接时判定。客户端 `bangumi_client`：UA 固定 `robinqu/RSSRipple`，token 走 `runtime_config.bangumi_api_key`（env `BANGUMI_API_KEY` 或设置页覆盖），API 基础地址走 `settings.bangumi_api_base`（env `BANGUMI_API_BASE`，默认 `https://api.bgm.tv/v0`，可指向镜像/mock），无独立启用开关——token 即启用。

**Bangumi 关联条目扩展（系列图谱，`bangumi_relations`）**：Bangumi 一个 subject 就是一季，franchise/multi_season 合集包链接到基础条目（如「頭文字D First Stage」）只覆盖了第一季；本 pass 经关联条目端点（`bangumi_client.get_subject_relations` → `GET /v0/subjects/{id}/subjects`）把同 IP 的其余条目展开成完整系列图谱。**触发条件保持收敛**：仅 `is_batch` 且 `batch_scope ∈ {franchise, multi_season}` 且已链接 bangumi 身份的资源（种子取自互斥 FK、`resource_work_links` 与 franchise 包的合集成员；主列或身份袋中的 `bangumi:{id}` 均可），挂载点为 `_process_resource_metadata` 链接后（`bind_single_work_assignments` 之后、commit 之前），单集资源不增加任何 API 调用。从种子 BFS 遍历关系图（上限 24 节点），**续集/前传/主线故事节点入队前先过同 IP 闸门**（Bangumi 的续集边会跨 IP——頭文字D Final Stage → MFゴースト）：候选条目剥离季标记后的归一化基础名（`strip_season_from_title` + `normalize_title`，name/name_cn 双语都试）必须与种子作品基础名相等或互为包含（"Final Stage" 类无数字季标记但含基础名的仍通过）；未过闸门的节点不 upsert、不 link、**不再从其继续 BFS**（整个异 IP 子图被隔绝），空气日期阈值与日期排序也只在过闸节点集合内计算（异 IP 节点的更晚日期不再把阈值推过合法的无标记续集）。随后按关系类型 + 条目 platform 确定性映射（`classify_work_shape`，franchise 成员解析共用同一函数、relation 置 None 时仅凭 platform）：**续集/前传/主线故事** → 季作品（季号优先取条目自身标题季标记——`season_from_title` 含 Stage 英文序数；无标记条目仅在播出日期严格晚于全部有日期标记条目时按日期序续排季号——如 Final Stage 接 Fifth 之后得 6；仍不可定跳过记 warning，绝不猜）；**剧场版** → Movie 作品；**番外篇**（OVA/spinoff）→ 同合集 `season_number=0` 季作品——`(collection, season 0)` 槽位只能有一个作品：**按播出日期最早者占槽**；其余不同身份的番外条目 upsert 为各自单季壳合集（series_group，season_number=1）的作品，仍经 `resource_work_links` 链到资源（跨合集 links 合法，`sync_resource_collection` 按既有语义把 season 系 scope 的资源 `collection_id` 置空，如实反映跨合集）；**不同演绎/重制**（如 Legend 新剧场版 vs Third Stage）→ movie 形态时保持独立 Movie 条目（不合并不降级），TV 形态跳过。所有成员经既有 upsert 入口创建/链接（`create_or_update_series_from_external` 带 season_hint——`season_hint=0` 为合法特典季号，不再被真值链吞掉；电影走 `create_or_update_movie_from_external`），身份走 bangumi 逐季粒度，creator-wins 主 id、身份袋与人工编辑保护不变；**创建季作品前先按 `(目标合集, season_number)` 查成员复用**（命中且成员无冲突 bangumi 身份时仅补身份入袋，不新建重复季作品）；袋命中作品带陈旧默认季号（成员解析对无季标记条目的 s1 猜测）时，图谱的确定性季指派会**订正 `season_number`**（人工编辑过的除外、目标槽位被占除外）；同样地，种子作品的季号若出自该默认猜测（标题带限定词但无季标记——`_season_is_default_guess`，season 0 的 OVA 形态证据除外），不计入 marked/日期证据集合并可作为普通节点重新入图接受指派（`_assign_seasons` 只见过滤后的证据种子）。落入单成员 `series_group` 壳合集的成员经 `try_absorb_shell_collection` 并入目标合集；占用作品的合集若为**同基础名的 auto 合集**（series_group/franchise_pack，归一化基础名相等或互为包含，多成员也允许——两者本就是同一 IP 的重复物）经 `try_absorb_same_name_collection` 整个并入（身份袋/aliases 去重合并、季槽位冲突的成员留在原合集、有 `manually_edited_fields` 的不吸收只记 warning）；**两个吸收函数删除合集行前都会把 park 在其上的 `FileResource.collection_id` 显式重挂到存留合集**（ORM 的删除置空只覆盖当 session 加载的作品成员，资源不重挂会留下悬空外键）；解析到**其他真实合集**的作品保留但不链接（s0 槽位占用后的番外壳作品除外——跨合集 link 是如实表达）。资源侧经 `resource_work_links` 建立多关联（`source="auto"`，manual/llm 行永不动；种子作品自身也补 link——multi_season 覆盖度键读 links 表），随后 `sync_resource_collection` 沉淀 `resource.collection_id`（franchise 不碰）、`batch_seasons` 取并集镜像（含 season 0，只增不减）。**仅扩展、不降级**：关系拉取或单成员展开失败静默记日志，原链接判定完全保留，幂等可重跑。图谱扩展之后紧跟一次幂等的簇级绑定复跑（`bind_hint_clusters`）：torrent 检测期的首跑发生在 metadata 链接之前，看不到图谱新建/改判的作品——复跑让未绑簇（如 "Initial D Fifth Stage" 经季标记层命中 s5 成员）对新成员解析，并把已绑 auto 行的 season 镜像到改判后的作品季号（如 Final Stage s1→s6）。

**关联形态与成员解析的配套规则（franchise 成员解析 / 簇绑定侧）**：①franchise 合集标题（`_pack_title`）只用清洗后的作品名——优先 `search_title`/`title_cn`，否则原始标题经发布信息 token 清洗（剥括号块、全N季/季范围/季标记、语言/字幕/清晰度/编码分节丢弃），含「全六季.日语.英文字幕.[BD][1080p][AV1]」的原始串禁止成为合集名；②franchise 成员解析与簇绑定的外部命中在 upsert 前**先查本地库**（`find_local_work_for_entity`：身份袋反查 + 归一化标题精确/模糊匹配 ≥ `AUTO_LINK_THRESHOLD` 85，两道拒绝守卫——候选已携带同 source 不同 id 即"同 IP 不同条目"绝不复用，以及标题数字 token 一致才接受——"Extra Stage 2" 不会并入 "Extra Stage"），命中仅补身份入袋（creator-wins 不抢主 id）、不新建重复行；web fallback 保持 identity-only 语义（只补身份，内容以主源为准）；③形态分类两处统一走 `classify_work_shape`（TV→季作品、剧场版→Movie、番外/OVA platform→s0 季作品，bangumi matched_entity 新增随实体携带的 `_platform` 键供成员解析判形态，落库前 pop）；④**带限定词但无数字季标记的 TV 条目**（"頭文字D Final Stage"——`single_season_entry` 只证明"是一季"不证明"是哪一季"）在 upsert 内不再享受单季默认 s1：`_has_unresolved_title_qualifier` 命中时 season 保持 None——合集中有多成员或单成员+限定词时挂合集待确认（袋身份），无合集可归时落各自单季壳合集（season_number=1），季号由系列图谱的确定性指派补正；`_resolve_collection_member` 复用成员前另加同 source 身份冲突守卫（成员已携带同 source 不同 id——含合成 `{id}#s{N}` 的基形式比对；wikipedia 豁免——不同 pageid 是同作品的不同语言版本）——冲突则身份挂合集、绝不合并进错误作品；⑤**图谱季槽位两阶段接纳（C1）**：季节点 upsert 前先跑"既有作品季号纠正"阶段（按节点指派订正默认猜测/簇提示落的错位季号，滚动至 fixpoint 最多 3 轮——`_correct_work_season`，人工编辑与槽位占用守卫不变），再逐节点 upsert 新季作品（纠正分支保留为兜底重试）——"Fifth 槽位被 Final 的簇提示落位占住、Final 随后被改判 s6"的序列下 Fifth/Final 都落位，无重试丢失；⑥**混合 franchise 包合集挂载（C2）**：LLM 精判绑定电影后不再整体跳过 `link_franchise_pack`——跳过条件收紧为纯电影包（精判自身把全电影作品包的 `batch_scope` 升级为 `movies`，franchise 判定分支天然排除），混合包（电影已绑、TV 簇 hint-only）照常建立/复用清洗标题的 franchise 合集并把全部关联作品（含已绑电影）挂入，`resource.collection_id` 沉淀；⑦**同资源同日期电影去重（C4，`dedupe_resource_movies`，挂在系列图谱扩展与二次簇绑定之后）**：成员解析（web fallback 英文标题）与系列图谱（日文标题）可能为同一部电影各建一行且跨语言标题无法桥接——同一资源 links 的电影作品中，**同 IP**（共享合集或 Round A 基础名 family：归一化基础名相等/互为包含）且**日期证据一致**（`release_date` 年份相等；web fallback 行无内容字段时改用其绑定文件路径的 `[YYYY]` 目录年份，单一取值才算）的不同身份行经既有去重合并机器（`_merge_movie_group`：assignments/links/decisions/agent works 重指向、身份袋并集、删重复行）合并——存留方为**频道主源身份行**（随后按 bangumi > tmdb > wikipedia > douban > mal > anilist > imdb > exa_web 次序），creator-wins 恢复存留方主 id/主 source（合并机器的 canonical 主 id 重选被回滚）；**无日期证据绝不合并**；⑧**无季证据 TV 成员不占主合集季槽位（D1）**：franchise 成员解析中，没有任何季证据（标题无季/Stage 数字标记——注意后缀表会剥但不能映射数字的 "Final Stage" 类词在此处只算限定词）的 TV 形态成员**不得**以默认 s1 挂入主合集——只有标题与合集基础名**原文**（不剥季）相等或包含于合集基础名的真基础名条目（"头文字D"/"Initial D" vs 双语合集名"头文字D Initial D"，`_is_pack_base_name`）才允许占 (collection, s1)；其余落限定名壳合集（series_group）并照常 link——否则占位者会让真首季滞留壳合集、季标记层把整季文件错绑到占位作品（"Initial D Battle Stage" 占 s1 → First Stage 26 文件误绑的录制事故）。bangumi/逐季粒度身份经 `classify_work_shape` 落 s0 的番外行为不变；⑨**季标记层限定词兼容（D2，纵深防御）**：season marker tier 选中成员前校验簇 hint 与候选成员标题的限定词残量（标题 token 减去合集名 token 与 hint 基础名 token 的差集；只有映射得出数字的季标记才被剥离——`_strip_numbered_marker`，"Final Stage" 不剥）——成员残量与 hint 残量必须相等（双方皆空=基础名作品，不受限），否则拒绝该层匹配（不绑，hint 保留）。

**Bangumi 手动搜索列表模式**：`POST /metadata/search`（UI 手动搜索，`listing=True`）在 `source=bangumi` 时不走 search-then-judge 收敛管线，直接 `list_bangumi_subjects`（复用 `_bangumi_queries` 查询清洗，多查询合并去重，limit 20，仍限 `type=[2]` 动画分类），每个 subject 映射一条薄候选（身份 `bangumi:{id}`、标题/年份/海报/简介 + `metadata.subject_season` 标注条目自身季标记供前端对照 season_hint），全量返回、不调 LLM；剧场版 platform 的条目 content_type=movie（受请求的 content_type 边界过滤）。作品刷新管线（`refresh_work_by_source`）保持 `listing=False` 的确定性 pick-one 行为不变。列表候选被选定后，preview/apply 检测到 bangumi 身份且候选 metadata 缺 `episode_list` 时按需展开——`build_entity_for_subject`（复用 `_build_matched_entity`：详情+剧集端点，season 取作品自身 `season_number`）补齐完整 entity 再比对（`_expanded_candidate_values`，展开失败保留薄候选、diff 相应变少，apply 内部 preview 复用同一份展开值不重复取网）。
...
4. **落库**：季作品 upsert 在 `episode_list` 存在时调用 `upsert_episodes`（按 `(series_id, season, episode)` 幂等 upsert title/air_date；只增不删；季作品只收本季条目，逐季源实体的条目标到其 `season_number`）。**连续编号防护**：季作品的清单若不以第 1 集起始（前序季连续编号特征）且作品 `number_of_episodes` 已知，超出 集数+2 容差的条目被丢弃并记 warning（防御 `sort` 续号污染）；以第 1 集起始的清单整体可信（开播中作品可超出滞后集数）。存量污染修复走 `scripts/bangumi_episode_repair.py`（dry-run 默认 + `--apply`；在线模式以 bangumi 季内 `ep` 为权威重建清单——删界外行、补缺失行、仅填 NULL/占位标题与日期，顺带用权威清单长度修复滞后 `number_of_episodes`（尊重人工编辑）；`--offline` 仅对"整列续号且行数等于集数"的作品做平移 rebase）。作品单季化后 `series.seasons`/`number_of_seasons` 两列退役为惰性孤儿、**永不写入**——实体的逐季数据改用于：惰性创建缺失季作品时的季级字段（该季 `number_of_episodes`/`start_date`/`end_date` 取自父条目 `seasons` 项与 `episode_list` 子集）与上述 Episode 落库；旧 `seasons_overwrite_allowed` 防退化 guard 随列退役从 metadata_service 删除。
5. **覆盖率评测与回填**：`scripts/wikipedia_seasons_eval.py`（dry-run 默认）对所有 wikipedia 链接的 TVSeries 逐部抓取 + 解析并输出覆盖率汇总；`--apply` 复用同一 `evaluate_series` 读路径执行写回填（批量提交 + `upsert_episodes`）。防退化 guard 已**内联进脚本**（解析季数少于既有行即 `[guard-skip]` 不写入），仅服务季拆分迁移前的存量行——对 wikipedia 主源 legacy 作品取代 `series_seasons_backfill.py` 的 seasons 回填；完成 `season_split_migration.py` 后该回填路径失去意义（新季作品不再携带 seasons 列数据）。**日期回填**走 `scripts/wikipedia_airdate_backfill.py`（dry-run 默认 + `--apply`）：对缺 `start_date`/`end_date` 的 wikipedia 身份季作品重抓页面跑 `parse_season_air_dates`，经 `_work_start_date`/`_work_end_date` 同一语义派生本季日期，只填 NULL 字段、尊重 `manually_edited_fields`；身份解析覆盖 `wikipedia:{lang}:{pageid}`（剥离 `#s{N}`）、legacy 裸 pageid/标题形式（zh→en→ja 探测）、`wikipedia_url`（`/wiki/` 与 `?curid=` 两种形式）与身份袋；链接版本页无日期时依次回退 zh/ja langlink 对应页、`<标题> (アニメ)`/` (動畫)` 动画子页面探测、同合集兄弟季作品的 locator（共享系列级页面），最终回退 bangumi 袋/主身份的按 id 精确查询（季粒度首播日期）。**遗留 wikipedia 身份重扫**走 `scripts/bangumi_rescan.py`（dry-run 默认 + `--apply`）：对 `external_source='wikipedia'` 的季作品经 `refresh_work_by_source` 单一管线以 bangumi 源重扫（只填空字段、尊重人工编辑、candidate 身份按自身 source 入身份袋——bangumi 主源未命中时回退的 mal/imdb/wikipedia 身份同样入袋，已属他作品的报 dedup candidate 不抢），并顺带把 `source` 列与 id 自声明前缀不一致的身份袋脏行重挂到声明源；bangumi 搜索限 TV 动画分类，电影不在范围；宿主机直连数据库运行时需 `--llm-base-url` 覆盖容器内地址、`POSTER_CACHE_DIR` 指向宿主可写目录再人工同步进容器卷。
...
**回退证据边界（2026-09-06）**：上述“首个有产出”指按 URL 注册表解析、通过频道白名单之后仍有候选；外站结果不能耗尽本轮查询，搜索返回的自声明 source/id 不可信。排序后最多五条即为 judge 和出口验证共享的候选集合。found=true 必须通过 `_bind_selected_entity`：显式 URL（百分号解码后）只能指向该集合中的同一候选；无 URL 时必须有唯一精确 source/id；模型 id/source 与 URL 身份冲突、未知候选或缺身份均返回 `web fallback judge returned ungrounded identity`，沿现有 transient 路径重试且不缓存。不得把模型生成的另一个页面当证据。回退出口除季数/集数外亦剥离 `episode_list`、`single_season_entry`、`alt_external_ids`，避免绕过季集与身份袋约束；found=false 不携带 matched_entity。历史 exa_web 行不迁移，新判定不再凭无依据标题创建 exa_web 身份。缓存代际升至 6，旧 verdict 惰性失效；不会自动解除既有作品关联或覆盖人工编辑。
...
- **频道级定期刷新（原全局自动刷新已废除）**：Channel 新增 `metadata_refresh_enabled` / `metadata_refresh_interval_minutes`（NULL=默认 1440）/ `metadata_refresh_full_scope` 三列。开启后调度器按间隔入队 `refresh_channel_works`（stable key `channel-refresh:<id>`），handler 只做参数派生——source = 频道自身 `metadata_source`、`override_manual_edits=False`——作品选集经 `select_channel_works_for_refresh`（默认缺失门控：只选确有待填空字段的作品，谓词与 `metadata_search` 的 fill 字段表一致；`full_scope=True` 跳过门控刷全部关联作品），随后复用与手动刷新完全相同的 `refresh_work_by_source → search_metadata_candidates → apply_work_metadata → process_title_only` 单一管线执行——剧集作品刷新会把自身 `season_number` 作为 `season_hint` 传入搜索（bangumi 季感知 auto-link/judge 据此选对当季条目，不会把第一季条目身份错填进后续季作品），日期写回走与 upsert 一致的季级语义（`_work_start_date`/`_work_end_date`：season 粒度实体取自身日期；系列级实体仅在该季 `seasons` 条目/集日期子集有证据时填本季日期，第一季或可验证单季才回落实体级日期）；season-0 特典作品跳过刷新（跳过网络搜索前先做确定性 start_date 兜底：NULL 且未人工编辑时取同合集非特典季作品的最早 start_date 落库，定期刷新据此自然收敛存量）；candidate 身份已被他作品占用时 409 不抢、以 `identity_conflict` 结果上报（去重候选）。存量频道开关一律收敛为关闭（轻迁移）。
...
**已下载历史去重**：单集、电影和合集覆盖度的跨运行去重，除活动态与 `completed` 外，也把 `status=cancelled AND completed_at IS NOT NULL` 视为已占用下载槽位。该组合是 organize `move` 成功后移除下载器 torrent、保留任务历史的正常终态，不能因状态改为 cancelled 而重新下载相同作品/季；下载完成前取消（`completed_at IS NULL`）不占槽位，后续资源仍可重新派发。
...
- 文件清单确定性解析以 `torrent_inspect.analyze_torrent_files` 为唯一实现：频道抓取、下载完成后的 Magnet 补齐及 organize 历史快照回退均复用它；不得在 planner 内维护另一套季集正则。所有主视频作品形态（单集 TV、单部电影、season/multi_season/franchise/movies）均建立 assignment，LLM 仍只处理确定性层无法完成的合集歧义（`llm_refinement_needed` 门控：franchise 恒触发；report.is_batch 且未解析占比超阈值触发；**资源 `is_batch=True`（标题层已判合集）且 report 落 `unknown` 且主视频 ≥2 个也触发**——覆盖 Stage 命名目录等路径层无季集结构的场景）。scope 判定把 season 0（特典/SP 文件）排除在「季数」计数外：≥2 个非零季才判 multi_season，单季+SP 仍判 season 且 episode 范围只取真季集号（覆盖季集合保留 0，SP 文件的 file_parses 照常落 season=0 映射）；路径季集解析（`extract_season_episode_from_path`）中文件名分量的显式 `SxxEyy`（含 S00）优先于目录继承的季号，目录/文件名任意分量的英文序数 Stage 标记（`Second Stage`/`2nd Stage`）同样提供季号。存量误判（`batch_seasons` 含 0）走 `scripts/batch_sp_rescan.py`（dry-run 默认 + `--apply`，torrent 缓存离线重析重写 scope/范围/assignments，不联网、不调 LLM）。
...
- **簇级作品解析（`cluster_work_binding.bind_hint_clusters`，挂在 `maybe_inspect_torrent` 富化段末尾、franchise 链接之后；另在 bangumi 系列图谱扩展后做第二次幂等运行）**：torrent 富化 pass 已按顶层目录聚类把簇标题写入 `work_title_hint` 并逐文件解析 season/episode，但 auto 行此前不绑作品；本步骤按 distinct hint 分簇（空 hint 跳过），对每个簇解析唯一目标作品后整簇绑定该簇全部未绑 auto 行。解析顺序（先命中先用，拿不准一律不绑、hint 保留待人工/LLM）：①**同合集成员**——资源已关联作品（互斥 FK/links）或已 park 到合集时，在同合集成员中按归一化标题相等匹配（标题+aliases），三个层级依次为：精确标题/别名相等（标题证据）→ hint 等于合集自身标题/aliases 的基础名放宽（剥季目录如"作品X S02"，按季号众数严格选胜，**非标题证据**）→ **季标记层**（hint 自带季/Stage 标记且剥季基础名匹配**资源自身 park 合集**的主标题——相等或互为包含；兄弟壳合集的拉丁主标题会造成包含歧义，故只看 park 合集——由标记季号选成员；目录自带的 Stage 词是标题证据）；②**本地 FTS**——作品库搜索，仅接受 ≥ 既有自动链接阈值（`AUTO_LINK_THRESHOLD`=85）且最高分唯一的命中，且须过**双守卫**：**形态兼容**（仅路径中的**显式集号模式**——SxxEyy/Exx/第N话/`[NN]`——锁死 TV 证据并禁绑 Movie，目录名孤立数字或 LLM 提示集号不算；hint/路径带显式电影标记词 movie/电影/剧场版 = 电影形态；**单文件簇在 OVA/OAD/剧场版/Movie/Film 标记目录下不被锁死 tv**——弱数字证据下的 movie 裁决可绑；Movie 候选仅在簇无集数证据时可绑）与**基础名包含**（归一化基础形——剥括号标签、数字季 token 与 `:：·/` 等分隔标点，但保留 Stage 序数词——必须相等或互为包含，"Initial D Fifth Stage" 对 "Initial D Battle Stage" 基础名 fifth≠battle 拒绝；基础形**精确相等**的候选优先于仅包含的候选）；外加**续号歧义守卫**（`_sequel_ambiguous`：剥数字后基础名相关但原文尾数不同——"Initial D Battle Stage" vs "...Battle Stage 2"——视为续集兄弟而非同一作品，仅精确原文归一化相等可绑，否则不绑 hint 保留；季号 token "Show S02" 不算续号，季号选胜路径不受影响）；FTS 空结果回退全表扫描（ngram AND 匹配跨不过冒号类分隔符，与 `match_*_by_title` 同一先例）；③**外部搜索**——经频道 metadata 源 `process_title_only`（与 franchise 成员解析同一入口），仅接受 `found` 且非 ambiguous 且形态兼容的确定性结果，随后走既有 upsert 入口（TV 带簇季号作 `season_hint`——hint 自带的季/Stage 标记优先于路径季号众数；落不了季返回 None 即跳过；身份袋 creator-wins 与人工编辑保护不变）。**包内季号 vs 作品季号调和**：证据分两档——**标题证据**（精确成员标题匹配/过守卫的 FTS 命中/外部 found）说明"这个簇就是这个作品"，作品自身 `season_number` 优先于包内路径季号：不一致的行改写 season 为作品季号后绑定（episode 是季内集号保持不变，provenance 仍 auto，记 debug 日志，返回值 `BindOutcome.remapped` 暴露 remap 计数）；**纯季号选胜**（基础名放宽层）保持保守：不一致不绑记 warning。绑定 Movie 的行清空 season（电影无季）。**reconcile**：每次运行先扫已绑定 auto 行——作品的 `season_number` 在绑定后被修正时（如 franchise 链接先把 "Final Stage" 落 s1、系列图谱随后改判 s6），已绑 auto 行跟随改判镜像作品季号（llm/manual 永不动）。llm/manual 行永不动，已绑 auto 行不重复绑（幂等重跑）。绑定后补 `resource_work_links(source=auto)`、重算 `season_ranges`、`batch_seasons` 取并集（season 系 scope）并由 `sync_resource_collection` 沉淀合集身份；全部失败静默降级为 hint-only。
