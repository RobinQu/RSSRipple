...
- **字段类型与 operator 支持**：
  ...
  - 作品类型字段（`content_type`）是派生枚举字符串字段：值由资源互斥的作品 FK 派生——`series_id` 非空 → `"tv"`、`movie_id` 非空 → `"movie"`、`audio_work_id` 非空 → `"audio"`、三者皆空（未识别、仅挂 WorkCollection 的 franchise 包、以及作品单季化后 FK 全清的 links-only multi_season 包）→ 空值（适用标准空值语义，`is_empty` 可匹配）。走字符串字段求值路径；UI 限制取值为 `"tv" | "movie" | "audio"`。franchise 可混合 TV/电影/OVA 且按不变量清除 flat FK，不存在合法的单一枚举值，因此该字段对 franchise 必填门禁不适用；其形态由 `is_batch` + `resource_collection` 表达。links-only multi_season 包的必填字段**展示/门禁**经 `resource_work_links` 派生媒介（required_fields `_semantic_field_value`），但 DSL 求值本身只读 FK id、不感知 links。该派生只读 FK id，无需 eager-load作品关系，所有求值点均安全。
...
- **频道必填字段门控**：频道必填字段清单 `required_metadata_fields` 覆盖**全部 DSL 字段**（资源级字段以 DSL 字段名为目录键；作品字段按 `series.`/`movie.` 成对归入语义键 `rating`/`year`/`genre`/`is_anime`/`collection`；资源级 franchise 合集展示名走 `resource_collection` 键）。该清单下 Agent 的 `filter_config` 与各 work 的 `filter_overrides` 在保存时仅允许使用资源级字段 + 已声明字段映射的作品字段，违规 422；运行时还按资源形态检查全部适用必填值，缺任一字段即阻止 Agent 过滤/派发并进入 Channel 文件资源待确认。清单**强制且创建后只增不删**：代码强制基线永不可清除；基线＝基础必选五件套（`search_title/content_type/is_batch/year/is_anime`），`title_cn`/`title_en` 为可选字段；其中 `content_type` 对无单一媒介类型的 franchise 行不适用，`year/is_anime` 只适用于单作品行。TV 单季合集要求 `episode_start`/`episode_end`，跨季合集由 `batch_seasons` 覆盖校验（links-only 形态校验关联季作品 links 与派生缓存一致），AudioWork 不进入 TV 合集字段检查。**作品单季化目录变更**：`season` 键从形态必填退役为**可选目录键**（季号由季作品身份承载；资源级 `season` DSL 字段本身保留为解析证据，`applies_to` 限 tv_single/tv_season_batch）；`absolute_episode`/`episode_confidence` 两键**退役出目录**（字段仍在资源模型与本 DSL 中，可正常用于过滤与 `pick_preferences`，但不再能声明为必填）；存量频道声明里的这三键由启动轻迁移一次性移除（哨兵 `required_fields_per_season_v1`），此后用户可在只增不删策略下重新加入 `season`。`pick_preferences`（优选偏好）与 OrganizeRule filter 不受字段声明门控；偏好引用的非必选字段为空时仅视为该偏好未命中，不阻止资源。权威目录：`app/services/required_fields.py`。
- 现行基线变更：`title_cn`/`title_en` 均不再强制；启动轻迁移一次性移除旧基线遗留值，此后用户主动勾选仍按必填处理。
