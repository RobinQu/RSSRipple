...
| External | Transmission (RPC), LLM API (OpenAI-compatible chat/completions), TMDB API (metadata search), Exa AI Agent API (search fallback), feedparser |
...
APScheduler/cron → fetch_channel_resources(channel)
  ...
  → fetch_and_link_metadata (本地模糊 → TMDB → Exa AI Agent)
...
| Metadata 搜索 | TMDB API 搜索作品信息 + Exa AI Agent 结构化回退，返回 metadata JSON |
...
1. **多源 metadata 搜索**：TMDB（主源）→ Exa AI Agent（回退），结构化 API 优先于 AI 搜索。TMDB 提供中文标题支持，Exa 通过结构化输出兜底。
