...
- 数据库后端三选一：`sqlite+aiosqlite:///`（默认，WAL）、`sqlite+aioturso:///`（嵌入式 Turso，MVCC 并发写；不支持 FTS5，检索降级 LIKE；迁移走 `scripts/migrate_to_turso.py`）、`postgresql+asyncpg://`。锁/冲突重试设施对三者语义一致。
