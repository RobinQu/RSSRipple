...
- 合集资源（`is_batch=true`）：`episode` 固定为空，`episode_start/end` 尽力而为；**去重按内容覆盖度**（`_batch_coverage_key`：电影包→movie_id、单季包→(series_id, season)、跨季包→(series_id, batch_seasons 季集合)）——仅覆盖度已知且完全相同的多版本进与单集一致的冲突解决（ask → PendingDecision（episode 哨兵 -1）/ auto → LLM pick → 启发式；跨运行按同 agent 同覆盖度 active 任务跳过），覆盖度不同或未知（标题层无季号依据）各自派发不去重；franchise 包作品 FK 全清不进派发。`batch_scope` 细分（NULL=非合集；`season`/`multi_season`/`franchise`）：三层判定——标题 pre-parser（含"有季标记+无集号+整碟 token"规则，命中默认 scope=season）→ torrent 内容检测（通道 A：抓取期对所有 http(s) 直链资源先 `ensure_torrent_cached` 缓存 .torrent 落盘 `TORRENT_CACHE_DIR` 并写回 `torrent_file`（已有缓存复用、失败静默），再对 `is_batch=false` 资源、以及已判合集但信息不完整（`batch_scope` 为 NULL，或 scope=`season` 缺 `episode_start/end`）的资源跑 `maybe_inspect_torrent`（复用缓存不重复下载、bencode 解析清单、纯函数 `analyze_torrent_files` 判 scope 并补齐集数范围，single/unknown 不降级既有合集判定，multi_season/franchise 把覆盖季集合持久化到 `batch_seasons`；franchise 由 `franchise_service.link_franchise_pack` 匹配成员作品、get-or-create `franchise_pack` 来源 WorkCollection、资源挂 `collection_id` 且作品 FK 全清；通道 B 下载后修正为保留优化项））→ LLM（可选 `batch_scope` 键，仅不降级写入）。任务创建优先推送 `torrent_file` 本地字节（`resolve_torrent_payload`）。organize 按 scope 分流：season 维持缺集拒绝（期望集 episode_start/end → work.seasons → 本地文件清单推导 min..max 区间）、multi_season 按季分组校验（显式依据与文件清单区间皆无的季跳过）、franchise 落 unclassified 待人工。
...
- `PATCH /resources/{id}/episode` 修正集号：未显式发送 `season` 且有 absolute 集号 + 剧集逐季数据时服务端推导 season（显式值优先）；**先 commit 再入队**定向运行（绕过且不推进水位线）。`PATCH /resources/{id}` 人工修订解析字段（episode/season/absolute_episode/episode_start/end/is_batch/batch_scope，按显式发送更新，服务端强制合集不变量，发送集号字段置 `manual`，修订结果为合集且原 `ambiguous` 也置 `manual`，同样先 commit 再定向运行）；`GET /resources/{id}/files` 返回 torrent 文件清单（`{files, source}`，source 依次 torrent_cache/torrent_fetch/downloader/notification/none）。
...
- 关键环境变量：`DATABASE_URL`、`QUEUE_BACKEND`、`APP_ROLE`（`all` 默认 / `web` / `worker`，分进程部署）、`LLM_API_KEY/BASE_URL/MODEL`、`EXA_API_KEY`、`JINA_API_KEY`、`TMDB_API_KEY`、`BANGUMI_API_KEY`、`*_ENABLED` 数据源开关、`POSTER_CACHE_DIR`、`AUTH_ENABLED`（认证总开关，默认开）、`API_KEY`（可选静态引导 key）等（完整列表见子文档；`PLEX_URL`/`PLEX_TOKEN` 已移除，媒体服务器配置入库；内置整理常开，无 `ORGANIZE_ENABLED`）。
