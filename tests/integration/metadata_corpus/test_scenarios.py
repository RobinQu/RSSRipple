...
from tests.metadata_corpus.reporting import run_reported_scenario
...
async def test_production_reconstruction(scenario, tmp_path, corpus_report_dir):
    database_url = os.environ.get("CORPUS_TEST_DATABASE_URL") or f"sqlite+aioturso:///{tmp_path}/corpus.db"
    result = await run_reported_scenario(ROOT, scenario, database_url, corpus_report_dir)
