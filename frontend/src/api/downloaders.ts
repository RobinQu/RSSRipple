...
export const downloadersApi = {
  ...
  listTasks: (id: string, page = 1, pageSize = 20, sort?: string, status?: string) =>
    api.get<DownloadTask[]>(
      `/downloaders/${id}/tasks?page=${page}&page_size=${pageSize}${sort ? `&sort=${encodeURIComponent(sort)}` : ''}${status ? `&status=${encodeURIComponent(status)}` : ''}`,
    ),
};
